/**
 * Unified Describe Command - Command Protocol準拠版
 */
...
export class UnifiedDescribeCommand implements Command {
  ...
  async perform(env: CommandEnvironment, subCommand: string[]): Promise<void> {
    const functionId = subCommand.length > 0 && !subCommand[0].startsWith('--') ? subCommand[0] : '';
    const options = this.parseOptions(subCommand);
    
    const describeFn = describeCommand(functionId);
    await describeFn(options)(env);
  }
