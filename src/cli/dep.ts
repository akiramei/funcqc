...
interface DepCyclesOptions extends BaseCommandOptions {
  minSize?: string;
  format?: 'table' | 'json' | 'dot';
  verbose?: boolean;
}
...
/**
 * Detect circular dependencies in the call graph (cycles subcommand of dep)
 */
 ...
 */
function outputCyclesJSON(cycles: string[][], options: DepCyclesOptions, functionMap: Map<string, FunctionInfo>): void {
  const result = {
    summary: {
 ...
 */
function outputCyclesTable(cycles: string[][], options: DepCyclesOptions, functionMap: Map<string, FunctionInfo>): void {
