...
export class QualityScorerV16 {
  ...
  private calculateBaseScores(functionInfo: FunctionInfo): EnhancedQualityMetrics['base'] {
    ...
    const maintainabilityScore = Math.max(0, Math.min(100, metrics.maintainabilityIndex || 100));
    ...
    if (complexity > this.thresholds.complexity.warning) {
      complexityScore -=
        (complexity - this.thresholds.complexity.warning) *
        this.thresholds.complexity.warningPenalty;
    }
    if (complexity > this.thresholds.complexity.critical) {
      complexityScore -=
        (complexity - this.thresholds.complexity.critical) *
        this.thresholds.complexity.criticalPenalty;
    }
  ...
  private convertEvaluationToScore(evaluation?: NamingEvaluation): number {
    ...
    switch (evaluation.rating) {
      ...
      case 2:
        return 60; // Partially Correct
      case 3:
        return 20; // Inappropriate
      default:
        return 80; // Fallback
    }
  }
  ...
  ): EnhancedProjectQualityScore['componentScores'] {
    const count = functionScores.length;

    const totals = functionScores.reduce(
      (acc, { metrics }) => ({
  ...
  private calculateProjectStatistics(
    ...
    const functionsWithEvaluations = functions.filter(func => evaluations.has(func.id)).length;
   ...
   * Get top problematic functions
   */
  private getTopProblematicFunctions(
    functionScores: Array<{ function: FunctionInfo; metrics: EnhancedQualityMetrics }>
  ): EnhancedProjectQualityScore['topProblematicFunctions'] {
    return functionScores
      .map(({ function: func, metrics }) => {
        ...
        if (metrics.base.maintainabilityScore < this.thresholds.maintainability.warning) {
          ...
          problemScore +=
            (this.thresholds.maintainability.warning - metrics.base.maintainabilityScore) * 2;
        }
  ...
  private generateRecommendations(
    ...
    const poorNamingFunctions = functionScores.filter(
      ({ metrics }) => metrics.composite.finalNaming < 70
    ).length;
    ...
    const poorTypeSafetyFunctions = functionScores.filter(
      ({ metrics }) => metrics.enhanced.typeSafetyScore < 70
    ).length;
