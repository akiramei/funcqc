...
export class SessionManager {
  ...
  async getSessionSummary(sessionId: string): Promise<SessionSummary> {
    ...
    try {
      ...
      const sessionFunctions = functionsResult.rows;
      ...
      const completedFunctions = sessionFunctions.filter((f: any) => f.status === 'completed').length;
      const progressPercentage = totalFunctions > 0 
        ? Math.round((completedFunctions / totalFunctions) * 100) 
        : 0;

      // Calculate effort
      const estimatedEffort = opportunities.reduce((sum: number, opp: any) => {
        const effort = (opp.metadata as any)?.estimatedEffort || 0;
        return sum + effort;
      }, 0);

      const actualEffort = sessionFunctions
        .filter((f: any) => f.status === 'completed')
        .reduce((sum: number, func: any) => {
          const effort = (func.metadata as any)?.actualEffort || 0;
          return sum + effort;
        }, 0);

      // Count patterns
      const patterns = opportunities.reduce((acc: Record<string, number>, opp: any) => {
