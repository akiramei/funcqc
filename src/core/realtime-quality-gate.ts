...
import { StructuralAnalyzer, StructuralMetrics, StructuralAnomaly } from '../utils/structural-analyzer.js';
...
export class RealTimeQualityGate {
   ...
   */
  async evaluateCode(code: string, context?: { filename?: string; existingBaseline?: ProjectBaseline }): Promise<QualityAssessment> {
   ...
   */
  generateImprovementInstruction(violations: QualityViolation[], structuralAnomalies: StructuralAnomaly[] = []): string {
    ...
    for (const violation of criticalViolations) {
      switch (violation.metric) {
        case 'cyclomaticComplexity':
          instructions.push(`Split this function - complexity is ${violation.value} (target: <${violation.threshold})`);
          break;
        case 'linesOfCode':
          instructions.push(`Function too long (${violation.value} lines) - extract helper methods`);
          break;
   ...
   */
  private async performAnalysis(code: string, filename: string): Promise<Omit<QualityAssessment, 'responseTime'>> {
    ...
    const improvementInstruction = (violations.length > 0 || structuralAnomalies.length > 0)
      ? this.generateImprovementInstruction(violations, structuralAnomalies)
      : undefined;

    return {
      acceptable: violations.filter(v => v.severity === 'critical').length === 0 && 
      ...
      if (stats && stats.isReliable) {
        ...
        if (anomaly.isAnomaly) {
          violations.push({
            metric: metricName,
            value,
            threshold: stats.currentMean + (this.config.warningThreshold * stats.currentStandardDeviation),
  ...
  private detectStaticViolations(metrics: QualityMetrics): QualityViolation[] {
    ...
    if (metrics.cyclomaticComplexity > staticFallback.cyclomaticComplexity) {
      violations.push({
        ...
        zScore: 0,
        severity: metrics.cyclomaticComplexity > staticFallback.cyclomaticComplexity * 1.5 ? 'critical' : 'warning',
   ...
   */
  private generateMetricSuggestion(metric: keyof QualityMetrics, _value: number, zScore: number): string {
   ...
   */
  private calculateStructuralScore(metrics: QualityMetrics, structuralMetrics?: StructuralMetrics): number {
  ...
  private updateMetricBaseline(metrics: QualityMetrics): void {
    ...
    for (const [metricName, value] of metricEntries) {
      if (!this.baseline.metrics.has(metricName)) {
        this.baseline.metrics.set(metricName, new StreamingStats({
