...
class NoCacheCallGraphAnalyzer extends CallGraphAnalyzer {
  ...
  protected resolveImportedSymbolBaseline(
    ...
    if (moduleSpecifier.startsWith('./') || moduleSpecifier.startsWith('../')) {
      ...
      const projectRoot = (this as any).findProjectRoot();
    ...
    for (const ext of extensionCandidates) {
      targetSourceFile = (this as any).project.getSourceFile(resolvedPath + ext);
      if (targetSourceFile) {
        break;
      }
    }
    ...
    if (targetSourceFile) {
      ...
      if (decls && decls.length > 0) {
        for (const decl of decls) {
          if ((this as any).isFunctionDeclaration(decl)) {
